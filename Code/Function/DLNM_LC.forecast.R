#' DLNM_LC.forecast: Forecast mortality using a fitted DLNM-LC model
#' 
#' @param DLNM_LC_model Output object from \code{DLNM_LC()}, containing \code{final_LC}, \code{baseline_LC},
#'   and iteration-wise \code{dlnm_coef}.
#' @param cb_matrix_test Cross-basis matrix for the forecast horizon.
#' @param wave_data_test Optional wave covariates for the forecast horizon. If
#'   \code{NULL} or empty, the wave contribution is set to zero.
...
#'   \item \code{DLNM_LC_final.parameter}: final (converged in algorithm) LC parameters and fitted time-series model for \code{k_t}.
#'   \item \code{DLNM_LC_final.forecast}: forecasts from the full DLNM-LC model.
#' }

DLNM_LC.forecast = function(DLNM_LC_model, cb_matrix_test, wave_data_test, kt_model_type, forecast_step){
  if (nrow(cb_matrix_test) != forecast_step){
    stop("The dimension of new data does not match the forecasting step.")
  }
  n_cb = ncol(cb_matrix_test)
  cb_matrix_test = as.matrix(cb_matrix_test)
  has_wave = !is.null(wave_data_test) && ncol(as.data.frame(wave_data_test)) > 0
  wave_data_test = if (has_wave) as.matrix(wave_data_test) else NULL

  # Forecasting parameters from Lee-Carter model
  axh = DLNM_LC_model$final_LC$a_x
  bxh = DLNM_LC_model$final_LC$b_x
  kt.fit = kt_model(DLNM_LC_model$final_LC$k_t, kt_model_type)
  ...
  cb_matrix.forecast <- lapply(DLNM_LC_model$dlnm_coef, function(iteration) {
    lapply(iteration, function(age_group) {
      as.numeric(age_group[1] + cb_matrix_test %*% age_group[seq_len(n_cb) + 1])
    })
  })
  ...
  wave.forecast = lapply(DLNM_LC_model$dlnm_coef, function(iteration) {
    lapply(iteration, function(age_group) {
      if (!has_wave) {
      ...
      as.numeric(wave_data_test %*% wave_coef)
    })
  })
  ...
  ax_ref = DLNM_LC_model$baseline_LC$a_x
  bx_ref = DLNM_LC_model$baseline_LC$b_x
  kt_ref.fit = kt_model(DLNM_LC_model$baseline_LC$k_t, kt_model_type)
  ...
  output = list(baseline_LC.parameter = list(ax_ref = ax_ref, bx_ref = bx_ref, 
                ...
                DLNM_LC_final.parameter = list(ax = axh, bx = bxh, kt.fit = kt.fit,
                                                 kt = kth),
                DLNM_LC_final.forecast = list(model.forecast = model.forecast,
