#' LL_model: Function for fitting Li--Lee model via product-ratio method.
#'
#' Implements the Li--Lee approach by:
#' (1) constructing the product (geometric-mean) mortality surface across regions,
#' (2) fitting a common Lee--Carter model on the product matrix to obtain shared \code{Bx} and \code{Kt},
#' (3) fitting a region-specific Lee--Carter model on each region's ratio-to-product surface to obtain \code{bx} and \code{kt}.
#'
#' @param dat_list A named list of mortality surfaces, one per region.
#'   All elements must have the same dimensions and aligned age/time ordering.
#'
#' @return A list with elements:
#' \itemize{
#'   \item \code{dat_list}: the input list.
#'   \item \code{Ax}: region-specific intercept vectors \code{Ax[[region]]} (common + individual).
#'   \item \code{Bx}, \code{Kt}: common time-varying factors.
#'   \item \code{bx}, \code{kt}: region-specific time-varying factors for each region.
#'   \item \code{common_Ax}: common intercept from the product-matrix LC fit.
#'   \item \code{individual_ax}: region-specific intercepts from the ratio-matrix LC fits.
#'   \item \code{std_mxt}: common standardised component from the product-matrix LC fit (log scale).
#'   \item \code{std_mxti}: region-specific standardised components from the ratio-matrix LC fits (log scale).
#'   \item \code{final_std_mxt}: region-specific standardised log-mortality combining common + region-specific parts.
#'   \item \code{log_fitted}: fitted log-mortality surfaces for each region (common + region-specific fits).
#' }
