#' kappa_fit: Fit and forecast common / region-specific kappa (K_t, k_t) from a DLNM-LL model
#'
#' @param DLNM_LL_model A fitted model object containing \code{final_LL} and \code{baseline_LL},
#'   each with elements \code{Kt} and \code{kt} (a list of region-specific \code{k_t} series).
#' @param forecast_step Integer forecast horizon (number of steps ahead).
#'
#' @return A nested list with forecasts for the final and baseline models:
#' \itemize{
#'   \item \code{final_kt}: lists of fitted models (\code{kt.fit}), forecasts (\code{kt.forecast}),
#'         and forecast means (\code{kt.pred}) for the final LL component.
#'   \item \code{base_kt}: analogous outputs (\code{kt.fit_ref}, \code{kt.forecast_ref}, \code{kt.pred_ref})
#'         for the baseline LL component.
#' }

kappa_fit = function(DLNM_LL_model, forecast_step){
  ## Final model
  Kt.df = data.frame(DLNM_LL_model$final_LL$Kt, DLNM_LL_model$final_LL$kt[[1]], 
                     DLNM_LL_model$final_LL$kt[[2]], DLNM_LL_model$final_LL$kt[[3]])
  ...
  ## Baseline model
  Kt.df_ref = data.frame(DLNM_LL_model$baseline_LL$Kt, DLNM_LL_model$baseline_LL$kt[[1]], 
                         DLNM_LL_model$baseline_LL$kt[[2]], DLNM_LL_model$baseline_LL$kt[[3]])
  ...
  Lisbon_fit_ref = auto.arima(ts(Kt.df_ref$Lisbon, frequency = 52), seasonal = T) # SARIMA
  ...
  Roma_fit_ref = auto.arima(ts(Kt.df_ref$Roma, frequency = 52), seasonal = T) # SARIMA
