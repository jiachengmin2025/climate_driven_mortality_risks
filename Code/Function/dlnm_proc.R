#' dlnm_proc: Fit DLNM across different age groups.
#'
#' @param dat Log-mortality rates
#'   with columns corresponding to age groups: 20--64, 65--74, 75--84, 85+.
#' @param cb_train Cross-basis matrix.
#' @param wave_data A list or data.frame containing wave covariates. Must include
#'   \code{<region>_hot_wave3} and \code{<region>_cold_wave3}.
#' @param region Character region label used to locate wave variables in \code{wave_data}.
#'
#' @return A list with elements:
#' \itemize{
#'   \item \code{DLNM_fitted}: a data.frame of fitted values on the log scale.
