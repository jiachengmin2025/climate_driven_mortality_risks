#' Guibert comparator: forecast mortality rates
#' Forecasts the Li--Lee time-varying factors and recombines them with the
#' fitted DLNM temperature-adjustment component.
#'
#' @param LL_fit Fitted Li--Lee model object from \code{LL_model()} fitted to
#'   non-temperature-attributable mortality rates.
#' @param dlnm_estimates_list Region-specific DLNM estimates returned by
#'   \code{Guibert.fit()}.
#' @param cb_matrix_test Named list of forecast-period UTCI cross-basis matrices.
#' @param forecast_step Integer forecast horizon.
#'
#' @return A list:
#' \itemize{
#'   \item \code{final_kt}: forecasted common and region-specific Li--Lee factors.
#'   \item \code{log_LL.forecast.list}: baseline Li--Lee forecasts on the log scale.
#'   \item \code{temp_adj_factor.forecast}: forecast temperature-adjustment factors.
#'   \item \code{cb.forecast}: forecast cross-basis linear predictors.
#'   \item \code{Guibert.forecast.list}: region-specific mortality-rate forecasts.
#' }
