...
UTCI_daily_mean = UTCI_daily_mean %>% mutate(Week) %>% select(Week, Roma) # 2 cols
UTCI_weekly_mean = UTCI_daily_mean %>% group_by(Week) %>%  summarise(Roma = mean(Roma))


UTCI_daily_mean = UTCI_daily_mean %>%
  mutate(Roma = if_else(row_number() <= 4, NA_real_, Roma))
...
lag0 = UTCI_daily_mean %>% group_by(Week) %>% 
  summarise(lag = Roma[7]) %>% select(lag) %>% slice(2:261)
lag0 = unlist(lag0[,1])
UTCI_ext_3[,1] = lag0
...
lag1 = UTCI_daily_mean %>% group_by(Week) %>% 
  summarise(lag = Roma[6]) %>% select(lag) %>% slice(1:260)
lag1 = unlist(lag1[,1])
UTCI_ext_3[,2] = lag1
...
lag2 = UTCI_daily_mean %>% group_by(Week) %>% 
  summarise(lag = Roma[5]) %>% select(lag) %>% slice(1:260)
lag2 = unlist(lag2[,1])
UTCI_ext_3[,3] = lag2
...
lag3 = UTCI_daily_mean %>% group_by(Week) %>% 
  summarise(lag = Roma[4]) %>% select(lag) %>% slice(1:260)
lag3 = unlist(lag3[,1])
UTCI_ext_3[,4] = lag3
...
lag4 = UTCI_daily_mean %>% group_by(Week) %>% 
  summarise(lag = Roma[3]) %>% select(lag) %>% slice(1:260)
lag4 = unlist(lag4[,1])
UTCI_ext_3[,5] = lag4
...
lag5 = UTCI_daily_mean %>% group_by(Week) %>% 
  summarise(lag = Roma[2]) %>% select(lag) %>% slice(1:260)
lag5 = unlist(lag5[,1])
UTCI_ext_3[,6] = lag5
...
lag6 = UTCI_daily_mean %>% group_by(Week) %>% 
  summarise(lag = Roma[1]) %>% select(lag) %>% slice(1:260)
lag6 = unlist(lag6[,1])
UTCI_ext_3[,7] = lag6
...
UTCI_ext.Roma = replace_na_with_mean(UTCI_ext_3)


## Save matrix
save(UTCI_ext.Roma, file = "Code/Crossbasis_matrix/cb_Roma.RData")
