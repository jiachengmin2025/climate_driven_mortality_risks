...
# Attiki
varknots <- equalknots(Attiki_prep_cb_rcp85, fun="ns",df = 2, degree = 3)
lagknots <- logknots(21, 2)
...
# Lisbon
varknots <- equalknots(Lisbon_prep_cb_rcp85,fun="ns",df = 2, degree = 3)
lagknots <- logknots(21, 2)
...
# Roma
varknots <- equalknots(Roma_prep_cb_rcp85,fun="ns",df = 2, degree = 3)
lagknots <- logknots(21, 2)
...
converged_LC.sim.rcp85 = Map(function(sim_sublist, axs_values) {
  # sublist
...
converged_LC.sim.rcp85 <- lapply(converged_LC.sim.rcp85, function(region) {
...
final_LC.sim.rcp85<- Map(function(region_converged, region_dlnm) {
...
}, converged_LC.sim.rcp85, dlnm.sum.sim)

age_group_names <- c("Y20_64", "Y65_74", "Y75_84", "Y_GE85")
final_LC.sim.rcp85 <- lapply(final_LC.sim.rcp85, function(region) {
...
# Generate error terms with the same structure as `final_LC.sim.rcp85` and `converged_LC.sim.rcp85`
error_terms <- generate_error_terms(final_LC.sim.rcp85, sd = 0.01)

# Add error terms to `final_LC.sim.rcp85`
final_LC.sim.rcp85 <- Map(function(data, error) {
  lapply(seq_along(data), function(i) {
    Map(`+`, data[[i]], error[[i]])  # Add error terms to corresponding data
  })
}, final_LC.sim.rcp85, error_terms)
...
# Add error terms to `converged_LC.sim.rcp85`
converged_LC.sim.rcp85 <- Map(function(data, error) {
  lapply(seq_along(data), function(i) {
    Map(`+`, data[[i]], error[[i]])  # Add error terms to corresponding data
  })
}, converged_LC.sim.rcp85, error_terms)
...
save(final_LC.sim.rcp85, file = "Results/Simulated_mortality_rates/final_LC_rcp85.RData")
