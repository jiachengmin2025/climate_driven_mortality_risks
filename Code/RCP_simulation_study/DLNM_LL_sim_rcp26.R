...
# Attiki
varknots <- equalknots(Attiki_prep_cb_rcp26, fun="ns",df = 3, degree = 3)
lagknots <- logknots(21, fun = "ns", df = 3)
...
# Lisbon
varknots <- equalknots(Lisbon_prep_cb_rcp26,fun="ns",df = 3, degree = 3)
lagknots <- logknots(21, fun = "ns", df = 3)
...
# Roma
varknots <- equalknots(Roma_prep_cb_rcp26,fun="ns",df = 3, degree = 3)
lagknots <- logknots(21, fun = "ns", df = 3)
...
converged_LL.sim.rcp26 = Map(function(sim_sublist, axs_values) {
  # sublist
...
converged_LL.sim.rcp26 <- lapply(converged_LL.sim.rcp26, function(region) {
...
final_LL.sim.rcp26 <- Map(function(region_converged, region_dlnm) {
...
}, converged_LL.sim.rcp26, dlnm.sum.sim)

age_group_names <- c("Y20_64", "Y65_74", "Y75_84", "Y_GE85")
final_LL.sim.rcp26 <- lapply(final_LL.sim.rcp26, function(region) {
...
# Generate error terms with the same structure as `final_LL.sim.rcp26` and `converged_LL.sim.rcp26`
error_terms <- generate_error_terms(final_LL.sim.rcp26, sd = 0.0001)

# Add error terms to `final_LL.sim.rcp26`
final_LL.sim.rcp26 <- Map(function(data, error) {
  lapply(seq_along(data), function(i) {
    Map(`+`, data[[i]], error[[i]])  # Add error terms to corresponding data
  })
}, final_LL.sim.rcp26, error_terms)
...
# Add error terms to `converged_LL.sim.rcp26`
converged_LL.sim.rcp26 <- Map(function(data, error) {
  lapply(seq_along(data), function(i) {
    Map(`+`, data[[i]], error[[i]])  # Add error terms to corresponding data
  })
}, converged_LL.sim.rcp26, error_terms)
...
save(final_LL.sim.rcp26, file = "Results/Simulated_mortality_rates/final_LL_rcp26.RData")
